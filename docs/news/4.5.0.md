...
* Adds `/fa set explosions`, a command for manually toggling explosion control in a faction.
* Adds config options `territoryBlockWindCharge` and `territoryBlockWindChargeWhenOffline` and `safeZoneBlockWindCharge` and `warZoneBlockWindCharge` and `wildernessBlockWindCharge`.
* Fixes dynmap description display issue, now properly reading from the config entry.

### 4.5.4 Changes

* Adds 26.2 support, fixing a loading error.
* Drops 1.21.11 support, nearly 3 months after 26.1 release.
* Fix: Send correct faction for relation wish notification to requesting faction.
* Fix: Allow taking books from lecterns in the wilderness.